...
def create_app():
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = (
        os.getenv("DATABASE_URL") or "sqlite:///test.db"
    )
    ...
    def create_parking():
        ...
        db.session.commit()
        return (
            jsonify({"id": new_parking.id, "address": new_parking.address}),
            HTTPStatus.CREATED,
        )
    ...
    def assign_client_to_parking():
        ...
        if client.credit_card is None or client.credit_card == "":
            return (
                jsonify({"error": "Кредитная карта не привязана."}),
                HTTPStatus.BAD_REQUEST,
            )

        if not client or not parking:
            return (
                jsonify({"error": "Клиент или Паркинг не существуют."}),
                HTTPStatus.BAD_REQUEST,
            )
    ...
    def delete_client_parking():
        ...
        client_parking: ClientParking = ClientParking.query.filter_by(
            client_id=client_id, parking_id=parking_id
        ).first()
        if not client_parking:
            return (
                jsonify({"error": "Клиент не найден на парковке."}),
        ...
        if parking:
            ...
            db.session.commit()
            return (
                jsonify({"message": "Клиент успешно выехал с парковки."}),
                HTTPStatus.OK,
            )
        else:
