...
def test_db():
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = (
        os.environ["DATABASE_URL"] or "sqlite:///test.db"
    )
...
def test_client_parking(client):
    ...
    response = client.post("/client_parkings", json={"client_id": 4, "parking_id": 2})
    assert response.status_code == 201
    ...
    response = client.post("/client_parkings", json={"client_id": 5, "parking_id": 2})
    ...
    response = client.post("/client_parkings", json={"client_id": 6, "parking_id": 3})
    assert response.status_code == 400
    ...
    response = client.post("/client_parkings", json={"client_id": 7, "parking_id": 2})
    assert response.status_code == 400
